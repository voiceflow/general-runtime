...
describe('runtime handler utils unit tests', () => {
  describe('canUseModel', () => {
    ...
    it('returns true if runtime plan supports GPT-4', () => {
      ...
      expect(canUseModel(AIModel.GPT_4_O as any, { plan: PlanType.PRO } as Runtime)).to.eql(true);
    ...
    it("returns false if runtime subscription entitlements doesn't supports GPT-4", () => {
      ...
      expect(canUseModel(AIModel.GPT_4_O as any, runtime)).to.eql(false);
    ...
    it('returns true if runtime subscription entitlements support GPT-4', () => {
      ...
      const gpt4OEnabledRuntime = {
        subscriptionEntitlements: [{ feature_id: 'feat-model-gpt-4-o', value: 'true' }],
      } as Runtime;
      ...
      expect(canUseModel(AIModel.GPT_4_O as any, gpt4OEnabledRuntime)).to.eql(true);
    });
  });
});
