...
describe('stateManagement manager unit tests', () => {
  ...
  describe('interact', () => {
    it('with state', async () => {
      ...
      const data = { params: { userID: 'user-id' }, body: {} };

      expect(await service.interact(data as any)).to.eql(handlerResult.trace);

      expect(services.session.getFromDb.args).to.eql([[data.params.userID]]);
      expect(_.get(data.body, 'state')).to.eql(session);
      expect(services.interact.handler.args).to.eql([[data]]);
      expect(services.session.saveToDb.args).to.eql([[data.params.userID, handlerResult.state]]);
    });
  ...
  describe('reset', () => {
    it('works', async () => {
      ...
      expect(services.session.saveToDb.args).to.eql([[data.params.userID, session]]);
    });
  });
});
