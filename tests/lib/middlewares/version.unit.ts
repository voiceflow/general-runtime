...
import Version from '@/lib/middlewares/version';
...
describe('version middleware unit tests', () => {
  describe('hasPermission', () => {
    it('throws', async () => {
      ...
      const middleware = new Version(services as any, {} as any);

      const req = { headers: { authorization: 'auth' }, params: { versionID: 'version-id' } };
      await expect(middleware.hasPermission(req as any, null as any, null as any)).to.eventually.rejectedWith('no permissions for this version');
    ...
    it('calls next', async () => {
      ...
      await middleware.hasPermission(req as any, null as any, next as any);
