...
describe('project middleware unit tests', () => {
  describe('attachID', () => {
    it('throws', async () => {
      ...
      const req = { headers: { authorization: 'auth', version: 'version-id' } };
      ...
      expect(api.getVersion.args).to.eql([[req.headers.version]]);
    });
    ...
    it('calls next', async () => {
      ...
      expect(services.dataAPI.get.args).to.eql([[req.headers.authorization]]);
      expect(api.getVersion.args).to.eql([[req.headers.version]]);
      expect(req.headers).to.eql({ authorization: req.headers.authorization, project: version.projectID, version: req.headers.version });
    });
  });
});
