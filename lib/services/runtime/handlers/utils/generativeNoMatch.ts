...
export const generateNoMatch = async (runtime: Runtime, projectType?: string): Promise<Output | null> => {
  ...
  return projectType === VoiceflowConstants.ProjectType.CHAT ? [{ text: response.data }] : response.data;
};
