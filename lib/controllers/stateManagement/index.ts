...
class StateManagementController extends AbstractController {
  ...
  async get(req: Request<{ userID: string }, any, { projectID: string; versionID: string }>) {
  ...
  async update(req: Request<{ userID: string }, State, { projectID: string; versionID: string }>) {
    await this.services.session.saveToDb(req.headers.projectID, req.params.userID, req.body);
    return req.body;
  }

  @validate({ HEADERS_PROJECT_ID: VALIDATIONS.HEADERS.PROJECT_ID })
  async delete(req: Request<{ userID: string; versionID: string }, any, { projectID: string; versionID: string }>) {
  ...
  async updateVariables(req: Request<{ userID: string }, Record<string, any>, { projectID: string; versionID: string }>) {
